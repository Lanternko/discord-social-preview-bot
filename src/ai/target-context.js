...
function buildTargetContextBlock(targets, opts = {}) {
  ...
  if (imitation) {
    lines.push("\n\n## 模仿對象參考");
    lines.push(
      "你被要求模仿下面這個人。這是一個獨立的新請求：如果剛才在玩角色扮演、某個梗或情境" +
        "（例如有人叫你扮某個角色），請先「跳出」那個情境——那不是這個人平常的語氣。" +
        "模仿時只根據下面「他最近真正的發言」，貼著他本人的用詞、句長、標點、顏文字和節奏走" +
        "（他平常多半講得很短、很跳，不要寫成華麗長文或作文腔）。可以直接化用他的口頭禪，這裡不受「不要複述」限制。",
    );
  } else {
    lines.push("\n\n## 被提到的人");
