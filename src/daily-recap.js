...
function buildRecapPrompt(stats, channelStats, guildName) {
  ...
  const lines = [
    [
      ...
      "- 下面列的熱門訊息是給你看全景用的，挑 3～4 則真的有梗的展開就好。其他頂多在某一段順口帶過一句，也可以整則完全不提——寧可少講幾件講得好笑，不要每則都交代一遍。",
      "- 不要每段都是「介紹這則＋我的感想」。至少有一段做別的事：把兩個人、兩個頻道、或同一個梗的兩種版本擺在一起比較，或是講到一半自己打臉（「我本來以為……結果根本不是」）。",
