...
需「管理伺服器」權限的斜線指令，per-guild 設定，重啟後仍保留（存在 `data/tier-settings.json`）：
...
- `/tier` → 顯示當前伺服器的詳細度設定
- `/tier level:標準` → 切換到標準
- 沒有「管理伺服器」權限的成員看不到這個指令（通常邀 bot 進來的那個人就有）
...
- **Slash 指令**：`/servers`（看 bot 在幾個伺服器）、`/debug-perms`（檢查頻道權限）、`/tier`（具管理伺服器權限者切換西寶詳細度，見 [上方](#切換回覆詳細度tier)）
