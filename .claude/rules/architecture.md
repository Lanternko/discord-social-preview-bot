...
- **Platforms** — [platforms/](../../src/platforms/) holds one builder per platform (`threads.js`, `instagram.js`, `bilibili.js`, `bahamut.js`, `ptt.js`). Each returns a `{ content?, embeds?, fallbackContent?, embedFallback? }` payload.
- **Preview dispatcher** — [preview.js](../../src/preview.js) `buildPreviewPayloads` is the top-level platform router. The `if` ladder order is load-bearing — see [routing.md](routing.md).
