...
- **圖片辨識**：@西寶 附圖（或 @ 她去回覆一則有圖的訊息）→ 鏈頭插一個 `deepseek-v4-flash-vision-exp` entry，圖以 `image_url` content block 掛在最後一個 user turn。**底下每一層都是瞎的**，所以 user turn 帶一行「你這次看不到，別假裝看得到」的註記（vision entry 送出前才換成「圖在下面」）；vision 模型改名/掛掉只是失去眼睛，回覆照常。實作 [src/ai/vision.js](src/ai/vision.js)，細節見 [ai-providers.md](docs/ai-providers.md)。
...
0. **Isolate parallel work in a `git worktree`.** The main checkout is shared by every process in that folder (other sessions, terminals, the running bot). Do NOT `git checkout` a feature branch there while others are active — spin up a dedicated worktree instead:
   ...
   ⚠️ **`npm test` in a worktree creates a fake `data/`.** The stores resolve their path from the module's own directory (`__dirname/../../data`), so a test run leaves a real `data/` **next to the `src/` that ran** holding fixtures (`sk-mykey`). Any later script you point at real guild data then reads the fixtures. Before touching live data from a worktree: `rm -rf data && ln -s "$(pwd)/../discord-social-preview-bot/data" data`. Same root cause is why `npm test` must **never** run in the main checkout — it would stamp those fixtures into the live `data/`; run it here on the branch instead. Full explanation in [deploy.md](docs/deploy.md#redeploy-after-merging).
