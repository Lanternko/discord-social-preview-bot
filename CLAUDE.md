...
- **Bilibili**: API-first via `https://api.bilibili.com/x/web-interface/view` (already in code, now wired). Success → custom embed **+ `videoAttachment`**（media.vxbilibili 的直鏈 mp4 → discord-io 下載後在封面 embed 下方上傳可播放影片，MIXED-style 同 Threads；放不下/停用/失敗退回純封面 embed）. Failure → vxbilibili fixer + OG recovery.
