...
- **Bilibili**: API-first via `https://api.bilibili.com/x/web-interface/view` (already in code, now wired). Success → custom embed **+ `videoAttachment`**（media.vxbilibili 的直鏈 mp4）：discord-io 下載後上傳可播放影片,並把 embed 換成 `videoAttachmentEmbeds`(無封面精簡版,因封面=影片 frame 會重複);放不下/停用/失敗才退回含封面 embed。MIXED-style 同 Threads video-only。Failure → vxbilibili fixer + OG recovery.
