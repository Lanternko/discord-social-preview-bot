...
- **Discord send failures must never propagate.** A reply/send throwing out of an event handler is an unhandled rejection, and Node's default `--unhandled-rejections=throw` then kills the gateway for every guild (real crash: 50013 when an AI reply echoed a user's `@everyone` without Mention Everyone perm). Mention replies go through `safeReply` (best-effort, swallows + logs `[mention]`); preview sends are already caught in `index.js`. Backstops: `process.on("unhandledRejection"/"uncaughtException")` in [src/index.js](src/index.js) log `[fatal]` — recoverable Discord errors (`isRecoverableDiscordError` in [src/discord-io.js](src/discord-io.js)) keep running, anything else exits for a clean watchdog restart.
