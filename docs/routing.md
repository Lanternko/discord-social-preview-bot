...
Local fallback 卡一律從 canonical `/@user/post/` 取作者（author 欄＋標題「@user 的 Threads 貼文」），並抓作者公開頁的 og:image 當頭像（只限 threads.com、帳號字元白名單、圖片須在 cdninstagram/fbcdn、3s 逾時、1h 快取；抓不到就不放）。被牆的三種情況（導回首頁／login wall／stub）在 probe.js 拋出時帶 `error.walled`，卡片說明改成「未登入看不到＋可能原因」，不再只寫「無法載入」；log 看 `threads viewer fallback walled=true`。
...
**pixiv 多頁作品走 embed 輪播、R18 走打碼卡**：phixiv 的 unfurl 只給第 1 頁、也不透露 R18。頁數／標題／作者／`xRestrict` 改打 pixiv 自己的 `www.pixiv.net/ajax/illust/<id>`（免金鑰免登入；phixiv 的 API 已停用並在回應裡指向這支）。第 2 頁以後的圖把首頁網址的 `_p0_` 換成 `_pN_` 即可，最多 `PIXIV_MAX_IMAGES`（預設 4）。**圖一定要走 phixiv 代理網址**——`i.pximg.net` 有 referer 防盜連，Discord 抓不到。R18（`xRestrict` > 0）的 `urls` 在未登入時整組是 null，改從 phixiv unfurl 的 og:image 取首頁再推算其餘頁。單頁全年齡作品維持 phixiv unfurl。grep `[pixiv]`。
...
**切片全景圖（X、pixiv、Threads）**：畫師常把一張寬圖切成 2～4 張等大直條上傳，Discord 相簿把 4 張排成 2×2 就斷掉了。尺寸全部相同且沒被截斷時（X 看 fxtwitter 的尺寸；pixiv 多打一次 `/ajax/illust/<id>/pages`，只在 2～4 頁時打；Threads 用 GraphQL 每張 `candidates[0]` 的寬高、只看不含影片的輪播——probe 路徑沒尺寸就不拼），payload 另帶 `panoramaImages`；`resolveOutgoing` → [src/panorama.js](../src/panorama.js) 下載後逐道檢查接縫，**每一道**都要過四關才拼：①連續（接縫差異 ≤ 圖內相鄰欄差異的 4 倍）②有紋理（純色邊緣不算證據）③不是畫框（接縫要明顯比「這張自己的左右緣」更像——共用邊框/背景的漫畫頁、差分會在這關被擋）④對得準（上下錯開幾 px 要變差）。門檻用實測校過：真全景切片 162/171 會拼（漏掉的是接縫落在純色區，只是退回相簿），真實多頁作品 0/56、隨機湊對 0/3192 誤拼（只用①② 時是 2 件與 7/3192）。Threads 拿 prod log 裡 270 則 2～4 張純圖輪播重跑：73 則等大（多是手機照），拼了 1 則且是真全景，0 誤拼——少見但成本只有下載＋約 0.5s CPU、不耗 token。過關就用 sharp 橫拼成 `panorama.jpg`（寬上限 4096）掛在主 embed；任何一關不過/下載失敗/超過上傳上限/guild 不允許 → 原相簿照發。沿用影片附件的閘門（`VIDEO_ATTACHMENT_*`）。R-18 走打碼附件，不拼。grep `[panorama]`。
