...
**切片全景圖（X、pixiv）**：畫師常把一張寬圖切成 2～4 張等大直條上傳，Discord 相簿把 4 張排成 2×2 就斷掉了。尺寸全部相同且沒被截斷時（X 看 fxtwitter 的尺寸；pixiv 多打一次 `/ajax/illust/<id>/pages`，只在 2～4 頁時打），payload 另帶 `panoramaImages`；`resolveOutgoing` → [src/panorama.js](../src/panorama.js) 下載後逐道檢查接縫，**每一道**都要過四關才拼：①連續（接縫差異 ≤ 圖內相鄰欄差異的 4 倍）②有紋理（純色邊緣不算證據）③不是畫框（接縫要明顯比「這張自己的左右緣」更像——共用邊框/背景的漫畫頁、差分會在這關被擋）④對得準（上下錯開幾 px 要變差）。門檻用實測校過：真全景切片 162/171 會拼（漏掉的是接縫落在純色區，只是退回相簿），真實多頁作品 0/56、隨機湊對 0/3192 誤拼（只用①② 時是 2 件與 7/3192）。過關就用 sharp 橫拼成 `panorama.jpg`（寬上限 4096）掛在主 embed；任何一關不過/下載失敗/超過上傳上限/guild 不允許 → 原相簿照發。沿用影片附件的閘門（`VIDEO_ATTACHMENT_*`）。R-18 走打碼附件，不拼。grep `[panorama]`。
