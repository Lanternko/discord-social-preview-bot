...
- **觸發要同時蓋「動詞貼著故事」和「動詞…的故事」兩種句型。** 上線第一天就漏了後者（`講一個關於X的故事` —— 主題卡在中間，`講` 跟 `故事` 根本不相鄰），整條技能沒跑、她用聊天人格隨手編了三句。主題型的 gap 有長度上限、不跨句、而且必須收在 `的`，才擋得住「你說的話根本不像故事」。另外有一條 `STORY_AUTHORED_RE` 擋「我剛剛講的故事」這種在談論既有故事的句子，其中的 `的` 是必要的 —— 少了它連「你講故事」這種真請求都會被吃掉。
