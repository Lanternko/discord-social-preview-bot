...
- **Voice reply** — [voice-reply.js](../src/voice-reply.js) adapts a slash interaction to the existing AI chain, appends speech-only Japanese formatting rules, and deliberately disables memory writes. [tts-client.js](../src/tts-client.js) calls the local Irodori service; [voice-message.js](../src/voice-message.js) sends Discord's raw `IS_VOICE_MESSAGE` payload. The existing mention path remains text-only.
