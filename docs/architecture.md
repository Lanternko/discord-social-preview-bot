...
- **Voice reply** — [voice-reply.js](../src/voice-reply.js) adapts a slash interaction to the existing AI chain but supplies a complete Japanese spoken persona, excludes text-chat history, and deliberately disables memory writes. Non-Japanese output gets one context-free repair pass. [tts-client.js](../src/tts-client.js) calls the local Irodori service; [voice-message.js](../src/voice-message.js) sends Discord's raw `IS_VOICE_MESSAGE` payload. The existing mention path remains text-only.
